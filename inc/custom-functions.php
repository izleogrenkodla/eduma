...
function thim_icomoon( $args, $arr ) {
	...
	if ( $args['field']['type'] == 'icomoon' ) {
		...
		$output .= '<div class="wrapper_icon thim_icomoon"><input type="hidden" name="' . $args['field_name'] . '" class="wpb_vc_param_value" value="' . $args['value'] . '" id="trace"/>
					<div class="icon-preview"><i class=" icon-' . $args['value'] . '"></i></div>';
		$output .= '<input class="search" type="text" placeholder="Search" />';
		$output .= '<div id="icon-dropdown-icomoon">';
		$output .= '<ul class="icon-list">';
		$n = 1;
		foreach ( $icons as $icon ) {
			$selected = ( $icon == $args['value'] ) ? 'class="selected"' : '';
		...
		$output .= '<script type="text/javascript">
                    jQuery(document).ready(function(){
                        jQuery(".thim_icomoon .search").keyup(function(){
                            // Retrieve the input field text and reset the count to zero
                            var filter = jQuery(this).val(), count = 0;
                            // Loop through the icon list
                            jQuery(".thim_icomoon .icon-list li").each(function(){
...
$theme_options_data = get_theme_mods();
if ( isset( $theme_options_data['thim_support_icomoon'] ) && $theme_options_data['thim_support_icomoon'] == '1' ) {
